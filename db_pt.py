...
def query_select(nama_table) :
    ...
    if nama_table == 't_jurusan' : 
        ...
        query = (f"SELECT {', '.join(unique_select)}\n"
                 ...
                 "join db_GOIconsV2.t_PerguruanTinggi tpt on tj.c_IdPerguruanTinggi = tpt.c_IdPerguruanTinggi ")
    elif nama_table == 't_jurusan_deskripsi' : 
        query = (f"SELECT tjd.*\n"
                f"FROM db_GOIconsV2.t_JurusanDeskripsi tjd\n"
                "JOIN db_GOIconsV2.t_Jurusan tj on tjd.c_IdJurusan = tj.c_IdJurusan")    
